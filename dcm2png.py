...
def dcm2png(dcm_path: Path, png_dir: Path, stem_suffix: str) -> Path | None:
    ...
    study_date: str = dataset.StudyDate if "StudyDate" in dataset else ""
    study_time: str = dataset.StudyTime if "StudyTime" in dataset else ""
    instance_number: int = int(
        dataset.InstanceNumber if "InstanceNumber" in dataset else ""
    )
...
def main(dcm_dir: Path, png_dir: Path, stem_suffix: str) -> None:
    ...
    for idx, dcm_path in track(
        enumerate(dcm_files), description="Converting dcm to png ..."
    ):
        try:
            png_path = dcm2png(dcm_path, png_dir, stem_suffix)
            if png_path is None:
                print(
                    f"{idx+1:^8d}/{len(dcm_files):^8d}: !!! failed to convert {str(dcm_path)}: no PixelArray found in DICOM !!!",
                    file=sys.stderr,
                )
                continue
            print(
                f"{idx+1:^8d}/{len(dcm_files):^8d}: converted {str(dcm_path)} to {str(png_path)} ...",
                file=sys.stderr,
            )
        except Exception as e:
            print(
                f"{idx+1:^8d}/{len(dcm_files):^8d}: !!! unexpected error on {str(dcm_path)} !!!",
                file=sys.stderr,
            )
